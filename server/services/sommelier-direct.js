/**
 * Enhanced Wine Processing Service
 * 
 * Processes wine lists directly line-by-line using GPT-4o
 * and stores each wine in the PostgreSQL database
 */
...
/**
 * Process a wine list and save wines to database
 * @param {string} wineListText - The complete wine list text
 */
 ...
 * @param {string} line - A single line from the wine list
 */
async function extractWineInfo(line) {
  try {
    console.log(`Processing: ${line.substring(0, 30)}...`);
...
Respond ONLY with a JSON object. No explanations.`
    ...
    // Fallback basic extraction if OpenAI fails
    const basicWine = {
      name: line
    };
    
    // Try to extract vintage with regex
